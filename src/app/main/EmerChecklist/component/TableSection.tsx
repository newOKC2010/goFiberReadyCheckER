...
import { formatDate } from '@/app/main/EmerChecklist/handler/handlerEmerChecklist';

interface TableSectionProps {
  data: EmerChecklistItem[];
  loading: boolean;
  totalCount: number;
  itemsPerPage: number;
  currentPage: number;
  ...
  onEdit: (item: EmerChecklistItem) => void;
}
...
export default function TableSection({
  ...
  const columns = [
    ...
    { key: 'name', label: 'ชื่อรายการ' },
    {
      ...
      render: (item: EmerChecklistItem) => (
        <span className={`px-3 py-1 rounded-full text-sm font-bold ${item.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {item.is_active ? 'ใช้งาน' : 'ปิดใช้งาน'}
        </span>
      )
    },
    { key: 'created_at', label: 'วันที่สร้าง', render: (item: EmerChecklistItem) => formatDate(item.created_at) },
  ];
