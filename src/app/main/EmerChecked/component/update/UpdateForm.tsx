  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="px-4 py-3 bg-gray-100 rounded-lg border border-gray-300"><p className="text-gray-800 font-bold">{checklistItem.name}</p></div>
      </div>
      {checklistItem.item_type === 'boolean' ? (
        <>
          <div>
        ...
        <div>
          <label className="block text-sm text-gray-700 mb-2 font-bold">ข้อมูล</label>
          <textarea value={note} onChange={(e) => setNote(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-900 placeholder:font-bold" rows={3} placeholder="กรอกข้อมูล..." />
        </div>
      )}
