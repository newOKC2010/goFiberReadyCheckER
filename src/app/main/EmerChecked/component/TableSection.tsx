...
import ActionButtons from '@/app/main/EmerChecked/component/ActionButtons';
...
export default function TableSection({ data, loading, totalCount, itemsPerPage, currentPage, totalPages, itemsPerPageOptions, showStaffColumn, showDeleteButton, showPrintButton, onItemsPerPageChange, onPageChange, onView, onPrint, onDelete }: TableSectionProps) {
  const columns = [
    ...
    { key: 'actions', label: 'จัดการ', render: (item: EmerCheckedItem) => (<ActionButtons item={item} showDelete={showDeleteButton} showPrint={showPrintButton} onView={onView} onPrint={onPrint} onDelete={onDelete} />) }
  ];
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 space-y-4">
      ...
      <Table columns={columns} data={data} loading={loading} getItemId={(item) => item.id} />
      <Pagination paginationData={{ count: itemsPerPage, total_count: totalCount, total_pages: totalPages, current_page: currentPage } as PaginationData} onPageChange={onPageChange} loading={loading} />
    </div>
  );
}
