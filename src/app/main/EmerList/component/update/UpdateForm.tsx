...
import { UpdateFormProps } from '@/app/main/EmerList/component/update/utils/types';
...
export default function UpdateForm({ item, loading, onSubmit, onCancel }: UpdateFormProps) {
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ emergency_id: item.id, license_plate_name: licensePlateName.trim(), active });
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <input type="text" value={licensePlateName} onChange={(e) => setLicensePlateName(e.target.value)}
          placeholder="กรอกทะเบียนรถ" disabled={loading} className={inputClass} />
      </div>
        ...
        <div className="flex items-center gap-4">
          {[true, false].map((val) => (
            <label key={String(val)} className="flex items-center gap-2 cursor-pointer">
              <input type="radio" checked={active === val} onChange={() => setActive(val)}
